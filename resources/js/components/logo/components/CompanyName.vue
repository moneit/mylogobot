<template>
    <g class="logo-group" id="logo-company-name" :fill="color" :style="{opacity: opacity}" :width="width" :height="height" :scale="scale">
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      logoSettings: { // for exception - white background and white company name color ->  temporarily make company name color as black
        type: Object,
        default: function() {
          return {};
        }
    ...
    mounted() {
      if (this.previewMode) {
        if (this.previewSettings.font.id) {
          ...
          this.$emit('api-completed');
          console.log('The preview font is not provided to get company name preview data.');
        }
      }
    },
    computed: {
      ...
      color() {
        if (this.previewMode) {
          if (
            ...
            this.logoSettings.backgroundColor.hex === '#FFFFFF' &&
            this.previewSettings &&
            this.previewSettings.color &&
            this.previewSettings.color.hex === '#FFFFFF'
          ) { // exception - white background and white company name color ->  temporarily make company name color as black
            return '#000000';
          }
    ...
    watch: {
      width(value) {
        if (this.previewMode) {
          ...
          this.$store.dispatch('companyName/updateWidth', value);
        }
      },
      ...
      height(value) {
        if (this.previewMode) {
          ...
          this.$store.dispatch('companyName/updateHeight', value);
        }
      },
      ...
      scale(value) {
        if (this.previewMode) {
          ...
          this.$store.dispatch('companyName/updateScale', value);
        }
      },
    }
  }
