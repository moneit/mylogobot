<template>
    <svg xmlns="http://www.w3.org/2000/svg"
         ...
         fill="none"
    >
        <preview-fail v-if="previewFail"></preview-fail>
        <template v-else>
            <rect v-if="showBackground && backgroundColor" id="background" :width="width" :height="height" :fill="backgroundColor"></rect>
            <g class="scaler" :transform="transform">
                <container
                        preview-mode
                        :preview-settings.sync="containerPreviewSettings"
                        :preview-fail.sync="containerPreviewFail"
                        :transform="containerTransform"
                        @api-completed="containerApiCompleted = true"
                ></container>
                <logo-symbol
                        preview-mode
                        ...
                        @api-completed="symbolApiCompleted = true"
                ></logo-symbol>
                <company-name
                        preview-mode
                        :logo-settings="previewSettings"
                        :preview-settings.sync="companyNamePreviewSettings"
                        :preview-fail.sync="companyNamePreviewFail"
                        :transform="companyNameTransform"
                        @api-completed="companyNameApiCompleted = true"
                ></company-name>
                <slogan
                        preview-mode
                        :preview-settings.sync="sloganPreviewSettings"
                        :preview-fail.sync="sloganPreviewFail"
                        :transform="sloganTransform"
                        @api-completed="sloganApiCompleted = true"
                ></slogan>
            </g>
...
<script>
  ...
  import PreviewFail from './components/PreviewFail.vue';
  import LogoPlacementMixin from '../../mixins/LogoPlacement.js';
  import LogoAdjustScaleMixin from '../../mixins/LogoAdjustScale.js';
  import LogoPreviewRefreshMixin from '../../mixins/LogoPreviewRefresh.js';
  import LogoPreviewRendering from '../../mixins/LogoPreviewRendering.js';
  ...
  export default {
    ...
    mixins: [
      ...
      LogoPreviewRefreshMixin,
      LogoPreviewRendering,
    ],
    components: {
      Container,
      CompanyName,
    ...
    data() {
      ...
      return {
        containerPreviewFail: false,
        companyNamePreviewFail: false,
        sloganPreviewFail: false,
        ...
        containerApiCompleted: false,
        symbolApiCompleted: false,
        companyNameApiCompleted: false,
        sloganApiCompleted: false,
      }
    },
    mounted() {
    ...
    computed: {
      ...
      layout: {
        ...
        set(layout) {
          this.previewSetting.layout = layout;
        },
      },
      backgroundColor() {
      ...
      companyNameIsPlaced() {
        return !!this.companyNamePreviewSettings.text;
      },
      companyNameWidth() {
      ...
      sloganIsPlaced() {
        return this.sloganPreviewSettings.text;
      },
      sloganLineSpaceRate() {
      ...
      symbolScale() {
        if (this.symbolType.label === 'Icon') { // icons
          return this.symbolPreviewSettings.iconScale;
        } else { // initials
          return this.symbolPreviewSettings.initialsScale;
        }
      },
      symbolWidth() {
    ...
    methods: {
      ...
      //         let mediumWidth = (this.width + this.boundingWidth) / 2;
      //         let mediumHeight = (this.height + this.boundingHeight) / 2;
      //
      //         let xRatio = mediumWidth / this.containerWidth;
      //         let yRatio = mediumHeight / this.containerHeight;
      //
      //         let ratio = Math.max(xRatio, yRatio);
      //
      //         this.containerPreviewSettings.size = Math.floor(this.containerPreviewSettings.size * ratio);
      //
      //         this.updatePreviewSettings();
      //     }
      // },
      updatePreviewSettingsProp() {
        this.$emit('update:preview-settings-prop', this.previewSettings);
      },
      updateContainerPreviewSettingsProp() {
        this.$emit('update:container-preview-settings-prop', this.containerPreviewSettings);
      },
      updateSymbolPreviewSettingsProp() {
        this.$emit('update:symbol-preview-settings-prop', this.symbolPreviewSettings);
      },
      updateCompanyNamePreviewSettingsProp() {
        this.$emit('update:company-name-preview-settings-prop', this.companyNamePreviewSettings);
      },
      updateSloganPreviewSettingsProp() {
        this.$emit('update:slogan-preview-settings-prop', this.sloganPreviewSettings);
      },
      updatePreviewSettings() {
        this.updatePreviewSettingsProp();
        this.updateContainerPreviewSettingsProp();
        this.updateSymbolPreviewSettingsProp();
        this.updateCompanyNamePreviewSettingsProp();
        this.updateSloganPreviewSettingsProp();
      },
    },
    watch: {
      previewFail(value) {
        this.$emit('preview-fail', value);
      },
      previewSettings() {
        this.updatePreviewSettingsProp();
      },
      ...
      symbolPreviewSettings() {
        this.updateSymbolPreviewSettingsProp();
      },
      companyNamePreviewSettings() {
        this.updateCompanyNamePreviewSettingsProp();
      },
      sloganPreviewSettings() {
        this.updateSloganPreviewSettingsProp();
      },
    },
  }
